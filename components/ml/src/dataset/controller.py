...
def load_dataset(path_to_dataset_dir : str, 
    ...
    """
    Loads the VR Controller Dataset
    ...
    :param feature_names: list of features to import from the dataset. Can be any of 
                          ['ControllerPos','ControllerOrientation',
                          'HeadsetPos','HeadsetOrientation', 'Time']
    ...
    :param ignore_handedness_both: If true, bin files containing recordings with both controllers will be ignored while loading. Otherwise an exception will be thrown if a bin file with recordings from both controllers is being found.
    ...
    data_getters = {
        'RightControllerPos' : lambda x: map(itemgetter('Position'), x['RightControllerData']),
        'RightControllerOrientation' : lambda x: map(itemgetter('Orientation'), x['RightControllerData']),
        'LeftControllerPos' : lambda x: map(itemgetter('Position'), x['LeftControllerData']),
        'LeftControllerOrientation' : lambda x: map(itemgetter('Orientation'), x['LeftControllerData']),
        'HeadsetPos' : lambda x: map(itemgetter('Position'), x['HeadsetData']),
        'HeadsetOrientation' : lambda x: map(itemgetter('Orientation'), x['HeadsetData']),
        'Time' : lambda x: map(itemgetter('Time'), x['HeadsetData']),
    }
    dataset_instances = [ ]
    dataset_labels = [ ]
    ...
    for filename in files:
        with open(filename,'rb') as f:
            ...
            # check if ControllerPos/ControllerOrientation is define din feature_names
            requires_controller_pos_or_orientation = 'ControllerPos' in feature_names or 'ControllerOrientation' in feature_names
            # make sure, the bin file does not contain recordings from both controllers which would lead to ambiguity
            has_left = 'LeftControllerData' in sample_data
            has_right = 'RightControllerData' in sample_data
            if requires_controller_pos_or_orientation:
                if has_left and has_right:
                    if ignore_handedness_both:
                        continue
                    else:
                        raise Exception('WARNING: Bin file contains a recording with both controllers which is not supported')

            sample_handedness = 'left' if has_left else 'right'

            gesture_feature_names = [ ]
            for feature_name in feature_names:
                if feature_name == "ControllerPos":
                    feature_name = 'LeftControllerPos' if has_left else 'RightControllerPos'
                elif feature_name == "ControllerOrientation":
                    feature_name = 'LeftControllerOrientation' if has_left else 'RightControllerOrientation'
            ...
            if import_gesture and (handedness is None or handedness == sample_handedness):
                ...
                for feature_name in gesture_feature_names:
                    features = np.stack(list(data_getters[feature_name](sample_data)))
...
if __name__ == "__main__":
    ...
    ds = load_dataset(
        path_to_dataset_dir=r'C:/Users/giorgospap/Desktop/EKETA/INFINITY/controller-gesture-dataset/remove-unused',
        feature_names=['ControllerPos','ControllerOrientation'],
        gesture_names=['I','U','INFINITY', 'LessThan','O','L', 'e', 'M'],
        ##handedness='right',
